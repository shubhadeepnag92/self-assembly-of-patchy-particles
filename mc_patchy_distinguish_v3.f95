...
!
	...
	real		   :: boxl,yt,OEp,OEc,OEpc,rsqrt,rhex,rtwelve, R, T, RT, p_r
...
!	GENERATE THE PATCH POSITIONS 
	...
	do iatom = 1,natom
		...
		ah(iatom,2,1) = ap(iatom,1) + sgma*cos(3.14159) ; 
		ah(iatom,2,2) = ap(iatom,2) + sgma*sin(3.14159) ; ah(iatom,2,3) = ap(iatom,3)
	enddo !iatom	
	...
	!	CALCULATE ANGLE BETWEEN PATCHES OF DIFFERENT PARTICLES
			...
			do jatom = 1,natom
				if (iatom.ne.jatom) then
					...
					if(rsqrt.ge.sgma.and.rsqrt.le.sgma+1) then
						if(theta11.le.0.3.and.theta21.le.0.3) Ei = Ei - eps11*3
						if(theta11.le.0.3.and.theta22.le.0.3) Ei = Ei - eps11*3
						if(theta12.le.0.3.and.theta21.le.0.3) Ei = Ei - eps11*3
						if(theta12.le.0.3.and.theta22.le.0.3) Ei = Ei - eps11*3
					endif
...
!	CALCULATE ANGLE BETWEEN PATCHES OF DIFFERENT PARTICLES
			...
			do jatom = 1,natom
				if (iatom.ne.jatom) then
					...
					! write(33,*) imc,iatom,jatom,x(:),x(1)*x(:)
	...
	!write(32,*) theta22*57.2958,(-x(1)*uh(jatom,2,1) - x(2)*uh(jatom,2,2) - x(3)*uh(jatom,2,3)),x(:),uh(jatom,2,:)
					...
					if(rsqrt.gt.sgma.and.rsqrt.le.sgma+1) then
						!write(32,*) iatom,jatom,rsqrt,'Eureka',theta11*57.2958, theta12*57.2958, theta21*57.2958, theta22*57.2958
						if(theta11.le.0.3.and.theta21.le.0.3) then
							Ef = Ef - eps11*3.0; write(35,*) imc,'Ef',Ef
						endif
						if(theta11.le.0.3.and.theta22.le.0.3) then
							Ef = Ef - eps11*3.0; write(35,*) imc,'Ef',Ef
						endif
						if(theta12.le.0.3.and.theta21.le.0.3) then
							 Ef = Ef - eps11*3.0; write(35,*) imc,'Ef',Ef
						endif
						if(theta12.le.0.3.and.theta22.le.0.3) then
							 Ef = Ef - eps11*3.0; write(35,*) imc,'Ef',Ef
						endif
					endif
		...
		uh(iatom,:,:) = 0
			...
			if (iatom.ne.jatom) then
				...
				if(rsqrt.ge.sgma.and.rsqrt.le.sgma+0.895) then
					if(theta11.le.0.52.and.theta21.le.0.52) E_final = E_final - eps11*3
					if(theta11.le.0.52.and.theta22.le.0.52) E_final = E_final - eps11*3
					if(theta12.le.0.52.and.theta21.le.0.52) E_final = E_final - eps11*3
					if(theta12.le.0.52.and.theta22.le.0.52) E_final = E_final - eps11*3
				endif
