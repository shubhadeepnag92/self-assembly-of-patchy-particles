...
!	GENERATE THE ATOM POSITIONS RANDOMLY
	...
	!ap(1,1) = boxl*0.5; ap(1,2) = boxl*0.5; ap(1,3) = boxl*0.5
	!ap(2,1) = boxl*0.5+sgma+0.5; ap(2,2) = boxl*0.5; ap(2,3) = boxl*0.5
	!ap(3,1) = boxl*0.5; ap(3,2) = boxl*0.5+sgma+0.5; ap(3,3) = boxl*0.5
	!ap(4,1) = boxl*0.5+sgma+0.5; ap(4,2) = boxl*0.5+sgma+0.5; ap(4,3) = boxl*0.5
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.4) Ei = Ei - eps_drive
					...
					if(iatom.eq.4.and.jatom.eq.1) Ei = Ei - eps_drive
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.4) Ef = Ef - eps_drive
					...
					if(iatom.eq.4.and.jatom.eq.1) Ef = Ef - eps_drive
...
!	CALCULATE THE FINAL ENERGY
	...
	do iatom = 1,natom-1
		...
		do jatom = iatom+1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				!write(*,*) 'Eureka'
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) E_final = E_final - eps_patch
				if(cos(theta11).ge.cos(theta0).and.cos(theta22).ge.cos(theta0)) E_final = E_final - eps_patch
				if(cos(theta12).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) E_final = E_final - eps_patch
				if(cos(theta12).ge.cos(theta0).and.cos(theta22).ge.cos(theta0)) E_final = E_final - eps_patch
			endif
	...
	write(32,*) imc,'Final Energy',E_final/(natom*eps_patch), E_final

	if ((E_final/natom*eps_patch).gt.0.9.and.(E_final/natom*eps_patch).lt.1.1) full_patch = full_patch + 1	

	endif

	enddo !imc
	
	write(*,*) 'full_patch',full_patch/nmc
