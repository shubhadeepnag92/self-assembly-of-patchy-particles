	...
	do iatom = 1,natom
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(iatom.eq.1.and.jatom.eq.13) E_ini = E_ini - eps_patch
				if(iatom.eq.13.and.jatom.eq.1) E_ini = E_ini - eps_patch        
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.13) then
						Ei = Ei - eps_patch; before_bond(iatom,jatom) = 1
                                !write(40,*) iatom,jatom,bias(iatom,jatom),'before'
					endif
					if(iatom.eq.13.and.jatom.eq.1) then
						Ei = Ei - eps_patch; before_bond(iatom,jatom) = 1
                                !write(40,*) iatom,jatom,bias(iatom,jatom),'before'
					endif
				endif
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.13) then
						Ef = Ef - eps_patch; after_bond(iatom,jatom) = 1
                                !write(40,*) iatom,jatom,bias(iatom,jatom),'after'
					endif
					if(iatom.eq.13.and.jatom.eq.1) then
						Ef = Ef - eps_patch; after_bond(iatom,jatom) = 1
                                !write(40,*) iatom,jatom,bias(iatom,jatom),'after'
...
! --------- WRITE THE TRAJECTORY AT INTERVAL OF 1000 STEPS ------------!
		...
		if(mod(imc,1000).eq.0) then
			...
			!enddo !i
			write(13,14) 'A',ap(1,1),ap(1,2),ap(1,3)
			write(13,14) 'P',ah(1,1,1),ah(1,1,2),ah(1,1,3)
			write(13,14) 'P',ah(1,2,1),ah(1,2,2),ah(1,2,3)
			write(13,14) 'B',ap(2,1),ap(2,2),ap(2,3)
			write(13,14) 'P',ah(2,1,1),ah(2,1,2),ah(2,1,3)
			write(13,14) 'P',ah(2,2,1),ah(2,2,2),ah(2,2,3)
			write(13,14) 'A',ap(3,1),ap(3,2),ap(3,3)
			write(13,14) 'P',ah(3,1,1),ah(3,1,2),ah(3,1,3)
			write(13,14) 'P',ah(3,2,1),ah(3,2,2),ah(3,2,3)
			write(13,14) 'B',ap(4,1),ap(4,2),ap(4,3)
			write(13,14) 'P',ah(4,1,1),ah(4,1,2),ah(4,1,3)
			write(13,14) 'P',ah(4,2,1),ah(4,2,2),ah(4,2,3)
                        write(13,14) 'A',ap(5,:)
                        write(13,14) 'P',ah(5,1,:)
                        write(13,14) 'P',ah(5,2,:)
                        write(13,14) 'B',ap(6,:)
                        write(13,14) 'P',ah(6,1,:)
                        write(13,14) 'P',ah(6,2,:)
			write(13,14) 'A',ap(7,:)
                        write(13,14) 'P',ah(7,1,:)
                        write(13,14) 'P',ah(7,2,:)
                        write(13,14) 'B',ap(8,:)
                        write(13,14) 'P',ah(8,1,:)
                        write(13,14) 'P',ah(8,2,:)
			write(13,14) 'A',ap(9,:)
                        write(13,14) 'P',ah(9,1,:)
                        write(13,14) 'P',ah(9,2,:)
                        write(13,14) 'B',ap(10,:)
                        write(13,14) 'P',ah(10,1,:)
                        write(13,14) 'P',ah(10,2,:)
			write(13,14) 'A',ap(11,:)
                        write(13,14) 'P',ah(11,1,:)
                        write(13,14) 'P',ah(11,2,:)
			write(13,14) 'B',ap(12,:)
                        write(13,14) 'P',ah(12,1,:)
                        write(13,14) 'P',ah(12,2,:)
                        write(13,14) 'C',ap(13,:)
                        write(13,14) 'P',ah(13,1,:)
                        write(13,14) 'P',ah(13,2,:)
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.13) E_final_patch = E_final_patch - eps_patch
					if(iatom.eq.13.and.jatom.eq.1) E_final_patch = E_final_patch - eps_patch
