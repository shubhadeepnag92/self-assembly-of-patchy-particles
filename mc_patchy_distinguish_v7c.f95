...
!
	...
	real		   :: eps11, eps12, eps22, sgmahex11, sgmahex12,sgmahex22, sgma, rsqr, sgmatwelve,sgmahex,stable_at_target
...
!	OPEN FILES
	...
	open(unit=13,file='traj.xyz')
       open(unit=33,file='stable_at_target.dat')
...
!	ALLOCATE DATA
	...
	RT = R * T; npatch = natom; rad_2_deg = 57.29; full_patch = 0; stable_at_target = 0
...
!	GENERATE THE ATOM POSITIONS RANDOMLY
	...
	ap(1,1) = boxl*0.3; ap(1,2) = boxl*0.3; ap(1,3) = boxl*0.5
	ap(2,1) = boxl*0.3+sgma+0.5; ap(2,2) = boxl*0.3; ap(2,3) = boxl*0.5
	ap(3,1) = boxl*0.3; ap(3,2) = boxl*0.3+sgma+0.5; ap(3,3) = boxl*0.5
	ap(4,1) = boxl*0.3+sgma+0.5; ap(4,2) = boxl*0.3+sgma+0.5; ap(4,3) = boxl*0.5
...
!	GENERATE THE PATCH POSITIONS 
	...
	ah(1,1,1) = ap(1,1) + sgma*0.5; ah(1,1,2) = ap(1,2) ; ah(1,1,3) = ap(1,3)
	ah(1,2,1) = ap(1,1) + sgma*0.5*cos(maxtheta) ; ah(1,2,2) = ap(1,2) + sgma*0.5*sin(maxtheta) ; ah(1,2,3) = ap(1,3)

	ah(2,1,1) = ap(2,1) - sgma*0.5; ah(2,1,2) = ap(2,2) ; ah(2,1,3) = ap(2,3)
	ah(2,2,1) = ap(2,1) + sgma*0.5*cos(maxtheta) ; ah(2,2,2) = ap(2,2) + sgma*0.5*sin(maxtheta) ; ah(2,2,3) = ap(2,3)
	
	ah(3,1,1) = ap(3,1) + sgma*0.5; ah(3,1,2) = ap(3,2) ; ah(3,1,3) = ap(3,3)
	ah(3,2,1) = ap(3,1) - sgma*0.5*cos(maxtheta) ; ah(3,2,2) = ap(3,2) - sgma*0.5*sin(maxtheta) ; ah(3,2,3) = ap(3,3)

	ah(4,1,1) = ap(4,1) - sgma*0.5; ah(4,1,2) = ap(4,2) ; ah(4,1,3) = ap(4,3)
	ah(4,2,1) = ap(4,1) - sgma*0.5*cos(maxtheta) ; ah(4,2,2) = ap(4,2) - sgma*0.5*sin(maxtheta) ; ah(4,2,3) = ap(4,3)

        !ap(1,1) = 3.20656991; ap(1,2) = 1.18863106; ap(1,3) = 0.264415741
        !ah(1,1,1) =  4.20656967; ah(1,1,2) = 1.18863106; ah(1,1,3) =  0.264415741
        !ah(1,2,1) =  3.20656633; ah(1,2,2) = 2.18863106; ah(1,2,3) =  0.264415741
        !ap(2,1) = 2.35668731; ap(2,2) =  2.22148108; ap(2,3) =  1.58818960
        !ah(2,1,1) = 3.35668731; ah(2,1,2) = 2.22148108; ah(2,1,3) =  1.58818960
        !ah(2,2,1) = 2.35668373; ah(2,2,2) = 3.22148108; ah(2,2,3) =  1.58818960
        !ap(3,1) = 0.666971445; ap(3,2) =  0.348300934; ap(3,3) =  5.87270260E-02
        !ah(3,1,1) = 1.66697145; ah(3,1,2) =  0.348300934; ah(3,1,3) =  5.87270260E-02
        !ah(3,2,1) = 0.666967809; ah(3,2,2) = 1.34830093; ah(3,2,3) =   5.87270260E-02
        !ap(4,1) = 1.27846766; ap(4,2) = 1.30442595; ap(4,3) =  0.261615992
        !ah(4,1,1) = 2.27846766; ah(4,1,2) = 1.30442595; ah(4,1,3) =  0.261615992
        !ah(4,2,1) = 1.27846408; ah(4,2,2) = 2.30442595; ah(4,2,3) =  0.261615992
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.3) E_ini = E_ini - eps_drive
					if(iatom.eq.2.and.jatom.eq.4) E_ini = E_ini - eps_drive
					if(iatom.eq.2.and.jatom.eq.1) E_ini = E_ini - eps_drive
					if(iatom.eq.3.and.jatom.eq.1) E_ini = E_ini - eps_drive
					if(iatom.eq.3.and.jatom.eq.4) E_ini = E_ini - eps_drive
					if(iatom.eq.4.and.jatom.eq.2) E_ini = E_ini - eps_drive
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.3) Ei = Ei - eps_drive
					if(iatom.eq.2.and.jatom.eq.4) Ei = Ei - eps_drive
					if(iatom.eq.2.and.jatom.eq.1) Ei = Ei - eps_drive
					if(iatom.eq.3.and.jatom.eq.1) Ei = Ei - eps_drive
					if(iatom.eq.3.and.jatom.eq.4) Ei = Ei - eps_drive
					if(iatom.eq.4.and.jatom.eq.2) Ei = Ei - eps_drive
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.3) Ef = Ef - eps_drive
					if(iatom.eq.2.and.jatom.eq.4) Ef = Ef - eps_drive
					if(iatom.eq.2.and.jatom.eq.1) Ef = Ef - eps_drive
					if(iatom.eq.3.and.jatom.eq.1) Ef = Ef - eps_drive
					if(iatom.eq.3.and.jatom.eq.4) Ef = Ef - eps_drive
					if(iatom.eq.4.and.jatom.eq.2) Ef = Ef - eps_drive
...
!---- write no of nth cycle,Energy at nth cycle,Energy of each particle after nth cycle  in 'energy.dat' file to plot---!
		...
		if(mod(imc,1000).eq.0) then
                         ...
                         write(11,*) imc,accp_patch/(accp_patch+rej_patch),1-(accp_patch/(accp_patch+rej_patch))
                         accp_patch = 0; rej_patch = 0
                endif
...
!	CALCULATE THE FINAL ENERGY
		...
		do jatom = 1,natom
			...
			if(rsqrt.ge.sgma.and.rsqrt.le.sgma+patch_width) then
				...
				if(cos(theta11).ge.cos(theta0).and.cos(theta21).ge.cos(theta0)) then
					...
					if(iatom.eq.1.and.jatom.eq.3) E_final = E_final - eps_drive
					if(iatom.eq.2.and.jatom.eq.4) E_final = E_final - eps_drive
					if(iatom.eq.2.and.jatom.eq.1) E_final = E_final - eps_drive
					if(iatom.eq.3.and.jatom.eq.1) E_final = E_final - eps_drive
					if(iatom.eq.3.and.jatom.eq.4) E_final = E_final - eps_drive
					if(iatom.eq.4.and.jatom.eq.2) E_final = E_final - eps_drive
	...
	enddo !iatom
        ...
        !E_final = E_final / real(imc*0.1)
        !write(*,*) imc,E_final
        if(abs(E_final).eq.1.0) stable_at_target = imc!*1.0
        if(abs(E_final).ne.1.0) then
                goto 100
        endif 
	!write(32,*) imc,E_final,stable_at_target!,d_m

		endif
        
	enddo !imc

100     write(33,*) stable_at_target 
        end program mc
